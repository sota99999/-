...
-- -----------------------------------------------------------------------------
-- レースのラップ（展開・ペース分析用）
--   200mごとのラップ列と、そこから導いた前後半・上り3F・前後傾(pace_diff)。
--   pace_diff = 前半1F平均 - 後半1F平均（秒）。正=後傾(前半遅後半速=瞬発力/上がり勝負)、
--   負=前傾(前半速後半遅=持続力/前残り勝負)。
--   scripts/collect_laps.py / scraper.py が結果ページから収集・投入する。
-- -----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS race_laps (
    race_id     TEXT PRIMARY KEY REFERENCES races(race_id),
    lap_seq     TEXT,      -- "12.4,10.9,11.5,..." 200mごとの秒
    n_laps      INTEGER,   -- ラップ本数
    first_half  REAL,      -- 前半の合計秒
